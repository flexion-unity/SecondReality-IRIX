#include <stdio.h>
#include <SDL2/SDL.h>
// On big-endian platforms (e.g. SGI IRIX / MIPS) the Scream Tracker 3 replayer
// cannot be used as-is because the .S3M format is entirely little-endian and the
// code contains many direct struct/pointer casts that assume little-endian byte
// order.  The stub implementations below provide a silent no-op replacement so
// that the demo can still build and run (without audio) on those platforms.
...
#define INITIAL_DITHER_SEED 0x12345000
    ...
    static const int8_t retrigvoladd[32] = { 0, -1, -2, -4, -8, -16, 0, 0, 0, 1, 2, 4, 8, 16, 0, 0, 0, 0, 0, 0, 0, 0, 10, 8, 0, 0, 0, 0, 0, 0, 24, 32 };

    static const uint8_t octavediv[16] = { 0, 1, 2, 3, 4, 5, 6, 7,

                                           // overflow data from "xvol_amiga" table
                                           0, 5, 11, 17, 27, 32, 42, 47 };

    static const int16_t notespd[16] = { 1712 * 16, 1616 * 16, 1524 * 16, 1440 * 16, 1356 * 16, 1280 * 16, 1208 * 16, 1140 * 16, 1076 * 16, 1016 * 16, 960 * 16,
                                         907 * 16, 1712 * 8,

                                         // overflow data from "adlibiadd" table
                                         0x0100, 0x0802, 0x0A09 };

    static const int16_t vibsin[64] = { 0x00,  0x18,  0x31,  0x4A,  0x61,  0x78,  0x8D,  0xA1,  0xB4,  0xC5,  0xD4,  0xE0,  0xEB,  0xF4,  0xFA,  0xFD,
    ...
    static const uint8_t vibsqu[64] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                                        0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                                        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

    static const int16_t vibramp[64] = { 0,    -248, -240, -232, -224, -216, -208, -200, -192, -184, -176, -168, -160, -152, -144, -136,
    ...
    static bool optimizeSampleDatasForMixer()
    {
#define SAMPLE_UNROLL_BYTES (512 + 2) /* ST3.21 value + 2 more for resampling interpolation window size */
        ...
        {
            ins_t * inst = &ins[i];
            if (inst->memsegOrig == nullptr || inst->length == 0) continue;
            ...
            if (loopLen512 > 0 && loopLen512 < 500) loopLen512 *= 1 + (500 / loopLen512);
            ...
            if (newPtr8 == nullptr) return false;
            ...
            else
            {
                ...
                if (is16bit)
                {
                    ...
                    for (uint32_t j = 0; j < SAMPLE_UNROLL_BYTES; j++)
                    {
                        ...
                        if (lastSmpVal16 > 0)
                        {
                            ...
                            if (lastSmpVal16 < 0) lastSmpVal16 = 0;
                        }
                ...
                else
                {
                    ...
                    for (uint32_t j = 0; j < SAMPLE_UNROLL_BYTES; j++)
                    {
                        ...
                        {
                            lastSmpVal8 -= 128 / 32;
                            if (lastSmpVal8 < 0) lastSmpVal8 = 0;
                        }
    ...
    static void setspd(chn_t * ch)
    {
        ...
        if (amigalimits)
        {
            if ((uint16_t)ch->aorgspd > aspdmax) ch->aorgspd = aspdmax;
    ...
    {
        if (volFlag) ch->achannelused |= 0x80;
    ...
    static int16_t stnote2herz(uint8_t note)
    {
        ...
        if (note == 254) return 0; // 0hertz/keyoff
    ...
    static int16_t scalec2spd(chn_t * ch, int16_t spd)
    {
        ...
        tmpspd = spd * C2FREQ;
        if ((tmpspd >> 16) >= ch->ac2spd) return 32767; // div error

        tmpspd /= ch->ac2spd;
        if (tmpspd > 32767) tmpspd = 32767;
    ...
    static int16_t roundspd(chn_t * ch, int16_t spd)
    {
        ...
        if ((newspd >> 16) >= C2FREQ) return spd; // div error
        ...
        if ((newspd >> 16) >= ch->ac2spd) return spd; // div error

        newspd /= ch->ac2spd;
        return (int16_t)newspd;
    }

    static int16_t neworder() // rewritten to be more safe
    {
        uint8_t patt;
        uint16_t numSep;
...
#define READ_2NDREAL_DECRYPTED_PATBYTE(i)                             \
    static_cast<uint8_t>(np_patseg[i] ^ ((i) + 2) ^ (((i) + 2) * 4)); \
    i++

    static int8_t getnote1()
    {
        uint8_t dat, channel;
        int16_t i;
        chn_t * ch;

        if (np_patseg == nullptr) return 255u;

        // added security that is not present in ST3
        if (np_pat >= patNum) return 255u;
    ...
    static void doamiga(uint8_t channel)
    {
            ...
            if (ch->ins <= 99) // 8bitbubsy: added for safety reasons
            {
                ...
                if (inst->type != 0)
                {
                    if (inst->type == 1) // sample
                    {
                        ...
                        v->insPtr = inst;
                        if (soundcardtype == SOUNDCARD_SBPRO) triggerVoice(v); // trigger sample here in SB mode
                    }
        ...
        if (ch->note != 255)
        {
            if (ch->note == 254) // ^^
            {
                ...
                ch->asldspd = 65535u; // 8bitbubsy: label jump bug causes this
            }
            else
            {
                // restart sample
                if (ch->cmd != 'G' - 64 && ch->cmd != 'L' - 64)
                {
                    ...
                    */
                    ...
                    else
                    {
                        ...
                        */
                        if (v->m_pos > v->m_origend && ch->astartoffset > 0 && v->m_loopflag) v->m_mixfunc = nullptr;
                    }
                }

                // 8bitbubsy: patch to kill voice on empty instruments
                if (ch->ins > 0 && ch->ins <= 99 && ins[ch->ins - 1].type == 0) voiceCut(channel);
    ...
    static void donotes()
    {
        ...
        {
            channel = getnote1();
            if (channel == 255) break; // end of row/channels
            ...
            if (ch->achannelused)
            {
                ...
                if (ch->cmd > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (ch->cmd != 'H' - 64 && ch->cmd != 'U' - 64 && ch->cmd != 'K' - 64 && ch->cmd != 'R' - 64) ch->avibcnt |= 128;
                    }
    ...
    static void docmd2() // tick >0 commands
    {
        chn_t * ch;
    ...
    static void dorow()
    {
        ...
        if (np_pat == 255) return; // 8bitbubsy: there are no patterns in the song!
    ...
    static void s_patloop(chn_t * ch)
    {
        ...
        if (patloopcount == 0)
        {
            ...
            if (patloopstart == -1) patloopstart = 0; // default loopstart
        }
    ...
    static void s_notecutb(chn_t * ch)
    {
        if (ch->anotecutcnt > 0)
        {
            if (--ch->anotecutcnt == 0) voice[ch->channelnum].m_speed = 0; // shut down voice (recoverable by using pitch effects)
        }
    }
    ...
    static void s_patterdelay(chn_t * ch)
    {
        if (patterndelay == 0) patterndelay = ch->info & 0x0F;
    }
    ...
    static void s_slidedown(chn_t * ch)
    {
        ...
        if (musiccount > 0)
        {
            ...
            ch->aspd += ch->info << 2;
            if ((uint16_t)ch->aspd > 32767) ch->aspd = 32767;
        }
        else
        {
    ...
    static void s_toneslide(chn_t * ch)
    {
        ...
        if (ch->aorgspd != ch->asldspd)
        {
            if (ch->aorgspd < ch->asldspd)
            {
                ...
                if ((uint16_t)ch->aorgspd > (uint16_t)ch->asldspd) ch->aorgspd = ch->asldspd;
            }
            else
            {
                ch->aorgspd -= toneinfo << 2;
    ...
    static void s_retrig(chn_t * ch)
    {
        ...
        if (soundcardtype != SOUNDCARD_GUS)
        {
            ...
            if (v->m_mixfunc == nullptr && (v->m_base8 != nullptr || v->m_base16 != nullptr))
    ...
    static void s_settempo(chn_t * ch)
    {
        if (!musiccount && ch->info >= 0x20) settempo(ch->info);
    }
    ...
    static void s_setgvol(chn_t * ch)
    {
        if (ch->info <= 64) setglobalvol(ch->info);
    }
...
#define LIMIT_MIX_NUM                                                                    \
    ...
    if (samplesToMix > (uint32_t)samplesToRender) samplesToMix = samplesToRender;        \
                                                                                         \
...
#define HANDLE_SAMPLE_END                             \
    ...
    if (realPos >= v->m_end)                          \
    {                                                 \
        v->m_mixfunc = nullptr; /* shut down voice */ \
        return;                                       \
    }
...
#define VOL0_OPTIMIZATION_LOOP                                            \
    ...
    mixRoutine mixRoutineTable[8] = { (mixRoutine)mix8bNoLoop,  (mixRoutine)mix8bLoop,  (mixRoutine)mix8bNoLoopIntrp,  (mixRoutine)mix8bLoopIntrp,
                                      (mixRoutine)mix16bNoLoop, (mixRoutine)mix16bLoop, (mixRoutine)mix16bNoLoopIntrp, (mixRoutine)mix16bLoopIntrp };
    ...
    {
        // LCG 32-bit random
        randSeed = static_cast<int32_t>(randSeed * 134775813);
        randSeed++;
    ...
    static void mixAudio(int16_t * stream, int32_t sampleBlockLength)
    {
        ...
        for (i = 0; i < 32; i++)
        {
            ...
            if (v->m_mixfunc != nullptr && v->m_pos < v->m_end) v->m_mixfunc(v, sampleBlockLength);
        }
...
#define SAMPLEMARKER_IDX(x) (((x) + sampleMarkerIndex) & (MARKER_MAX - 1))
    ...
    void st3play_SetMasterVol(uint16_t vol)
    {
        ...
        if (mastermul == 0) mastermul = 48;

        mixingVol = mastermul * mastervol * (512 + 64);
    }

    uint16_t st3play_GetMasterVol()
    {
    ...
    void st3play_SetInterpolation(bool flag)
    {
        ...
        for (uint8_t i = 0; i < 32; i++)
            voice[i].m_mixfunc = nullptr;
    }

    char * st3play_GetSongName()
    {
        return songname;
    }

    uint32_t st3play_GetMixerTicks()
    {
        if (audioRate < 1000) return 0;

        return sampleCounter / (audioRate / 1000);
    }

    // Read a little-endian 16/32-bit value from a byte pointer — safe on big-endian hosts.
    ...
    static bool loadS3M(const uint8_t * dat, uint32_t modLen, uint32_t startingOrder)
    {
        ...
        int16_t *smpWritePtr16;
        uint16_t patDataLen;
        uint32_t c2spd, i, j, offs;

        if (modLen < 0x70 || dat[0x1D] != 16 || memcmp(&dat[0x2C], "SCRM", 4) != 0) return false; // not a valid S3M

        memcpy(songname, dat, 28);
        songname[28] = '\0';

        signedSamples = (read_le16(&dat[0x2A]) == 1);

        ordNum = read_le16(&dat[0x20]);
        if (ordNum > 256) ordNum = 256;
        insNum = read_le16(&dat[0x22]);
        if (insNum > 100) insNum = 100;
        patNum = read_le16(&dat[0x24]);
        if (patNum > 100) patNum = 100;
        ...
        for (i = 0; i < insNum; i++)
        {
            ...
            offs = read_le16(&dat[0x60 + ordNum + (i * 2)]) << 4;
            if (offs == 0) continue; // empty

            ptr8 = (uint8_t *)&dat[offs];

            inst->type = ptr8[0x00];
            inst->length = read_le32(&ptr8[0x10]);
            inst->lbeg   = read_le32(&ptr8[0x14]);
            inst->lend   = read_le32(&ptr8[0x18]);
            ...
            c2spd = read_le32(&ptr8[0x20]);
            if (c2spd > 65535) c2spd = 65535;
            ...
            if (offs + inst->length >= modLen) inst->length = modLen - offs;

            if (inst->lend == inst->lbeg) inst->flags &= 0xFE; // turn off loop

            if (inst->lend < inst->lbeg) inst->lend = inst->lbeg + 1;

            if (inst->lend > inst->length) inst->lend = inst->length;

            if (inst->lend - inst->lbeg <= 0) inst->flags &= 0xFE; // turn off loop
        }
        ...
        for (i = 0; i < patNum; i++)
        {
            offs = read_le16(&dat[0x60 + ordNum + (insNum * 2) + (i * 2)]) << 4;
            if (offs == 0) continue; // empty

            patDataLen = read_le16(&dat[offs]);
            if (patDataLen > 0)
            {
                patdata[i] = (uint8_t *)malloc(patDataLen);
                if (patdata[i] == nullptr)
                {
                    st3play::Close();
                    return false;
                }
        ...
        for (i = 0; i < insNum; i++)
        {
            ...
            if (inst->length <= 0 || inst->type != 1 || dat[offs + 0x1E] != 0) continue; // sample not supported
            ...
            if (hasloop && inst->length > inst->lend) inst->length = inst->lend;

            inst->memsegOrig = (int8_t *)malloc(4 + (inst->length * bytesPerSample));
            if (inst->memsegOrig == nullptr)
            {
                st3play::Close();
                return false;
            }
            ...
            if (signedSamples)
            {
                if (is16bit)
                {
                    // S3M is little-endian on disk; read each sample with explicit LE decode
                    smpWritePtr16 = (int16_t *)inst->memseg;
                    for (j = 0; j < inst->length; j++)
                        smpWritePtr16[j] = (int16_t)read_le16(&dat[offs + j * 2]);
                }
                else
                {
                    memcpy(inst->memseg, &dat[offs], inst->length);
                }
            }
            else
            {
                if (is16bit)
                {
                    ...
                    smpWritePtr16 = (int16_t *)inst->memseg;
                    for (j = 0; j < inst->length; j++)
                        smpWritePtr16[j] = (int16_t)(read_le16(&dat[offs + j * 2]) - 32768u);
                }
                else
                {
                    for (j = 0; j < inst->length; j++)
        ...
        for (i = 0; i < 32; i++)
        {
            ...
            ch->apanpos = 0x77;
            if (chnsettings[i] != 0xFF) ch->apanpos = (chnsettings[i] & 8) ? 0xCC : 0x33;
            ...
            if (dat[0x35] == 252) // custom pannings follow, force GUS mode and set pans
            {
                ...
                if (pan & 32) ch->apanpos = ((pan & 0xF) << 4) | (pan & 0xF);
            }
        }
        ...
        mastermul = dat[0x33];
        fastvolslide = (read_le16(&dat[0x28]) == 0x1300) || (dat[0x26] & 0x40);
        ...
        mastermul &= 127;
        if (mastermul == 0) mastermul = 48; // default in ST3 when you play a song where mastermul=0

        // no mastermul in GUS, set to a low'ish value because GUS can be quiet (thus loud songs were made)
        if (soundcardtype == SOUNDCARD_GUS) mastermul = 32;

        if (dat[0x32] > 0) settempo(dat[0x32]);
        if (dat[0x30] != 255) setglobalvol(dat[0x30]);

        if (dat[0x31] > 0 && dat[0x31] != 255) setspeed(dat[0x31]);
        ...
        np_ord = startingOrder > ordNum ? 0 : static_cast<uint16_t>(startingOrder);
        neworder();
    ...
    */
    const int16_t fastSincTable[256 * 4] = {
        // Cubic Spline
        0,     16384, 0,     0,     -31,   16383, 32,    0,     -63,   16381, 65,    0,     -93,   16378, 100,   -1,    -124,  16374, 135,   -1,    -153,
        -1056, -260,  2738,  14952, -1046, -251,  2666,  15004, -1036, -242,  2596,  15055, -1025, -234,  2526,  15106, -1014, -225,  2456,  15155, -1002,
        -216,  2387,  15204, -990,  -208,  2319,  15251, -978,  -200,  2251,  15298, -965,  -192,  2184,  15344, -952,  -184,  2117,  15390, -939,  -176,
        2051,  15434, -925,  -168,  1986,  15477, -911,  -161,  1921,  15520, -896,  -153,  1857,  15561, -881,  -146,  1794,  15602, -866,  -139,  1731,
        15642, -850,  -132,  1669,  15681, -834,  -125,  1608,  15719, -818,  -118,  1547,  15756, -801,  -112,  1488,  15792, -784,  -105,  1428,  15827,
        -766,  -99,   1370,  15861, -748,  -93,   1312,  15894, -729,  -87,   1255,  15926, -710,  -81,   1199,  15957, -691,  -75,   1144,  15987, -671,
        -70,   1089,  16016, -651,  -65,   1035,  16044, -630,  -60,   982,   16071, -609,  -55,   930,   16097, -588,  -50,   878,   16121, -566,  -46,
        828,   16145, -543,  -41,   778,   16168, -521,  -37,   729,   16190, -497,  -33,   681,   16210, -474,  -30,   634,   16230, -450,  -26,   587,
        16248, -425,  -23,   541,   16265, -400,  -20,   497,   16281, -374,  -17,   453,   16296, -348,  -14,   410,   16310, -322,  -12,   368,   16322,
        -295,  -9,    327,   16334, -268,  -7,    287,   16344, -240,  -5,    247,   16353, -211,  -4,    209,   16361, -183,  -3,    172,   16368, -153,
...
namespace st3play
{
    bool FillAudioBuffer(int16_t * audioBuffer, int32_t samples)
    {
        ...
        while (a > 0)
        {
            ...
            b = a;
            if (b > samplesLeft) b = samplesLeft;
    ...
    bool PlaySong(const uint8_t * moduleData, uint32_t dataLength, bool useInterpolationFlag, uint32_t audioFreq, uint32_t startingOrder)
    {
        st3play::Close();

        memset(songname, 0, sizeof(songname));

        if (audioFreq == 0) audioFreq = 44100;
        ...
        if (mixBufferL == nullptr || mixBufferR == nullptr)
        {
            st3play::Close();
        ...
        if (!AudioPlayer::openMixer(audioRate, samplesLeft, totalSampleCount))
        {
            printf("openMixer failed!!\n");
            st3play::Close();
    ...
    void Close()
    {
        AudioPlayer::closeMixer();

        if (mixBufferL != nullptr)
        {
            free(mixBufferL);
            mixBufferL = nullptr;
        }

        if (mixBufferR != nullptr)
        {
            free(mixBufferR);
            mixBufferR = nullptr;
        }
        ...
        {
            if (ins[i].memsegOrig != nullptr)
            {
                free(ins[i].memsegOrig);
                ins[i].memsegOrig = nullptr;
            }
    ...
    {
        uint32_t samplePosition = AudioPlayer::getSamplePosition();
        ...
        {
            int half = (i1 + i0) / 2;
            if (sampleMarkers[SAMPLEMARKER_IDX(half)].sampleCountStart < samplePosition)
            {
                i0 = half;
            }
            else
    ...
    uint16_t GetOrder()
    {
        ...
        st3play::GetOrderRowAndFrame(&currentOrder, &currentRow, &currentFrame);
        return currentOrder;
    }
    ...
    int16_t GetPlusFlags()
    {
        ...
        switch (flags)
        {
            case 0:
                return -32;
            case 1:
                return currentRow - 64 < -32 ? -32 : currentRow - 64;
            case 2:
                return currentRow >= 32 ? -32 : currentRow;
            case 3:
                return static_cast<int16_t>(currentRow) < -32 ? currentRow : currentRow - 64;
        }
        return 0;
    }
}
